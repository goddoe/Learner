...
namespace CRVL
{
	...
	void make_HOG_128_64(cv::Mat _src, cv::Mat& _outputFeature)
    {
		Mat src;
		...
		Mat adaptive_example = src; // ROI영역인 clone matrix -> adaptive_thresholding 적용
		...
		resize(adaptive_example, adaptive_example, Size(50, 50));
		uchar* adaptive_example_data = (uchar*)adaptive_example.data;

		//        imshow("adap",adaptive_example);
		//        waitKey(0);
		//printf("here");
		_outputFeature = Mat(adaptive_example.rows * adaptive_example.cols, 1, CV_32FC1);
		float* _outputFeature_data = (float*)_outputFeature.data;



		//printf("here2");
		for (int _y = 0; _y < adaptive_example.rows; ++_y)
		{
			for (int _x = 0; _x < adaptive_example.cols; ++_x)
			{
				_outputFeature_data[_y*adaptive_example.cols + _x] = adaptive_example_data[_y* adaptive_example.cols + _x] / 255.0;
			}
		}
    }
	...
	void make_NoFeature_For_Crosswalk(cv::Mat _src, cv::Mat& _outputFeature)
	{
		resize(_src, _src, Size(100, 20));
		_outputFeature = Mat(_src.rows*_src.cols * 3, 1, CV_32FC1);
		for (int _y = 0; _y < _src.rows; ++_y)
		{
			for (int _x = 0; _x < _src.cols; ++_x)
	...
	void make_Sobel_For_Crosswalk(cv::Mat _src, cv::Mat& _outputFeature)
	{
		Mat grad_x, grad_y;
		Mat abs_grad_x, abs_grad_y;
		Mat tmp;

		resize(_src, tmp, Size(100, 10));
